...
export class AssetsService {
  ...
  constructor(
    ...
    @Inject(Constants.ArchivesModelToken)
    private readonly ArchivesModel: ArchiveModel,
    @Inject(Constants.FilesModelToken) private readonly filesModel: FileModel,
    @Inject(Constants.SectionsModelToken)
    private readonly sectionsModel: SectionModel,
    @Inject(Constants.CollectionsModelToken)
    private readonly collectionsModel: CollectionModel,
  ) {}
  ...
  async getSectionsByUser(userId, skip: number, limit: number, filter: number) {
    const query = this.sectionsModel.find({
      'contractInfo.contractor': userId,
      status: { $lte: filter },
    });
  ...
  async submitWork(submitedWork: SubmitWork) {
    ...
    for (const work of submitedWork.works) {
      ...
      if (section) {
        try {
          const commit = await section.addCommit(
            new CreateCommitDto(
              work.type,
              work.text,
              submitedWork.userId,
  ...
  async updateWeekly(updateCommand: UpdateCommand) {
    ...
    try {
      const fileList = await this.downloaderService.updateFilelist(
        updateCommand.fileListMark,
      );
      let count = 0;
      ...
      for (const fileName of Object.keys(fileList)) {
        const _promise = async () => {
          ...
          const archive = await this.ArchivesModel.getArchive(
            new CreateArchiveDto(fileName),
          );
            ...
            for (const rawText of rawTexts) {
              ...
              const infoIndex =
                archive.files.length > 0
                  ? archive.files.findIndex(f => f.name === rawText.name)
                  : -1;
              // let docModel = null;
              if (
                infoIndex !== -1 &&
                archive.files[infoIndex].hash === textHash
              ) {
                continue;
              }
              // else if (infoIndex !== -1) {
              ...
              const docModel = await this.filesModel.createFile(
                new CreateFileDto(rawText.name, filePath, rawText.fileType),
              );
              await docModel.mergeSections(sections);
              await archive.updateFileInfo(
                rawText.name,
                textHash,
                docModel._id,
                infoIndex,
      ...
      const fileListInfo = await this.ArchivesModel.findOne({
        dlName: 'file-list',
      }).exec();
